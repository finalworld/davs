...
ui.againBtn.addEventListener('click',()=>{state=null;const saved=localStorage.getItem('spenatstorm_display_name');const validSaved=saved?validateName(saved):'';if(validSaved)confirmedName=validSaved;ui.nameInput.value=confirmedName||'';ui.nameStatus.textContent=confirmedName?`Sparat namn: ${confirmedName}`:'';ui.nameStatus.className=confirmedName?'status-line ok':'status-line';ui.startBtn.disabled=!confirmedName;ui.confirmNameBtn.textContent=confirmedName?'Byt namn':'Bekräfta namn';ui.result.classList.remove('show');ui.start.classList.add('show');loadLeaderboard(ui.startLeaderboard,confirmedName);});
...
if (savedDisplayName) {
  const validSaved = validateName(savedDisplayName);
  if (validSaved) {
    confirmedName = validSaved;
    ui.nameInput.value = validSaved;
    ui.nameStatus.textContent = `Sparat namn: ${validSaved}`;
    ui.nameStatus.className = 'status-line ok';
    ui.startBtn.disabled = false;
    ui.confirmNameBtn.textContent = 'Byt namn';
  }
}
