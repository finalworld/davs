...
const ui = {
  ...
  againBtn: $('#againBtn'), joystick: $('#joystick'), joyKnob: $('#joyKnob'), toast: $('#toast'), settings: $('#settingsScreen'), settingsSoundBtn: $('#settingsSoundBtn'), joyLeftBtn: $('#joyLeftBtn'), joyRightBtn: $('#joyRightBtn'), closeSettingsBtn: $('#closeSettingsBtn'), zoomControls: $('#zoomControls'), zoomInBtn: $('#zoomInBtn'), zoomOutBtn: $('#zoomOutBtn'), zoomLabel: $('#zoomLabel'), zoomButtonsToggle: $('#zoomButtonsToggle')
};
...
function applySettingsUI(){
  ...
  if(ui.zoomControls){
    ...
    ui.zoomControls.classList.toggle('hidden',!showZoomButtons||!playing);
  }
...
canvas.addEventListener('pointermove',e=>{
  ...
  if(pinchPointers.size===2&&pinchStartDistance>10){
    ...
    setZoom(pinchStartZoom*(d/pinchStartDistance),false);
  }
});
...
function spawnEnemy() {
  ...
  const ang=Math.random()*Math.PI*2,rad=(Math.max(width,height)*.68)/cameraZoom+rand(80,180),bx=state.player.x+Math.cos(ang)*rad,by=state.player.y+Math.sin(ang)*rad;
...
async function gameOver(){
  ...
  ui.pause.classList.remove('show');ui.upgrade.classList.remove('show');ui.hud.classList.add('hidden');ui.joystick.classList.add('hidden');ui.zoomControls.classList.add('hidden');
...
async function loadLeaderboard(target, highlight=''){
  ...
  try{
    ...
    renderLeaderboard(target,scores,highlight);
    ui.startServerInfo.textContent='🌍 Världstopplista – gemensam för alla spelare.';
