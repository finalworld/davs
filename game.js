...
const ui = {
  ...
  againBtn: $('#againBtn'), joystick: $('#joystick'), joyKnob: $('#joyKnob'), toast: $('#toast'), settings: $('#settingsScreen'), settingsSoundBtn: $('#settingsSoundBtn'), joyLeftBtn: $('#joyLeftBtn'), joyRightBtn: $('#joyRightBtn'), closeSettingsBtn: $('#closeSettingsBtn')
};
...
function loadSettings(){try{const saved=JSON.parse(localStorage.getItem(SETTINGS_KEY)||'{}');if(typeof saved.audioOn==='boolean')audioOn=saved.audioOn;if(saved.joystickSide==='left'||saved.joystickSide==='right')joystickSide=saved.joystickSide;}catch(e){}applySettingsUI();}
function saveSettings(){try{localStorage.setItem(SETTINGS_KEY,JSON.stringify({audioOn,joystickSide}));}catch(e){}}
function applySettingsUI(){if(ui.settingsSoundBtn)ui.settingsSoundBtn.textContent=audioOn?'🔊 På':'🔇 Av';if(ui.joyLeftBtn&&ui.joyRightBtn){ui.joyLeftBtn.classList.toggle('active',joystickSide==='left');ui.joyRightBtn.classList.toggle('active',joystickSide==='right');}if(ui.joystick)ui.joystick.classList.toggle('joy-right',joystickSide==='right');}
...
function startGame() {
  ...
  if (matchMedia('(pointer: coarse)').matches) { applySettingsUI(); ui.joystick.classList.remove('hidden'); }
